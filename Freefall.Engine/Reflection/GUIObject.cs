...
namespace Freefall.Reflection
{
    public class GUIProperty
    {
        ...
        public virtual string Name { get { return _field.Name; } }
        public virtual Type Type { get { return _field.Type; } }
        public bool ReadOnly => !_field.CanWrite;
        ...
        public bool HasMixedValue
        {
            get
            {
                var result = _field.GetValue(targets[0]);

                for (int i = 1; i < targets.Length; i++)
                {
                    var temp = _field.GetValue(targets[i]);
        ...
        {
            if(IsList) return _field.Type.GetGenericArguments()[0];
            return Type;
        }
        ...
        {
            if (hashcode == 0)
                hashcode = (Depth, _field.DeclaringType.GetHashCode(), 0, _field.Name.GetHashCode()).GetHashCode();
        ...
        public virtual object GetValue()
        {
            ...
            for (int i = 0; i < targets.Length; i++)
                return _field.GetValue(targets[i]);
    ...
    public class GUIPropertyElement : GUIProperty
    {
        ...
        {
            hashcode = (Depth, _field.DeclaringType.GetHashCode(), elementIndex + 1, _field.Name.GetHashCode()).GetHashCode();
            return hashcode;
        }
        ...
        public override object GetValue()
        {
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                var list = _field.GetValue(targets[i]) as IList;
    ...
    public class GUIObject
    {
        ...
        private bool IsSerializable(Type type) => typeof(Assets.Asset).IsAssignableFrom(type) || Reflector.GetAttribute<SerializableAttribute>(type) != null;
